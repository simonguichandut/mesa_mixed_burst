...
! ***********************************************************************
      ...
      subroutine do_Kipp_Plot(s, id, device_id, &
         ...
         have_log_R = get1_yvec('log_R', log_R)
         if (.not. have_log_R .and. s% Kipp_switch_to_Column_Depth) then
            write(*,*) 'PGSTAR Kipp failed to find log_R (required for column_depth) in history data'
            ierr = -1
         end if
         if (ierr /= 0) return
         ...
         subroutine setup_column_yaxis
            ...
            if (ymax <= ymin) ymax = ymin+1
            dy = ymax - ymin
            if (s% Kipp_column_min /= -101d0) ymin = ymin + s% Kipp_column_margin*dy 
            if (s% Kipp_column_max /= -101d0) ymax = ymax - s% Kipp_column_margin*dy 
            call pgswin(xleft, xright, ymax, ymin) ! inverted y-axis so that surface is on top
            call pgscf(1)
            call pgsci(1)
         end subroutine setup_column_yaxis
            ...
            do
               ...
               if (step <= step_max .and. x <= xnext) then
                  ...
                  call draw_burn_for_step( &
                     pg, i_burn_type_first, i_burn_type_last, &
                     burn_max, burn_min, burn_type_cutoff, x, &
                     log_xmstar(step-step_min+1), star_M_center(step-step_min+1))
                  xnext = max(x - dx, xmin)
               end if
               pg => pg% next
            end do
         ...
         subroutine draw_burn_for_step( &
               pg, i_burn_type_first, i_burn_type_last, &
               bmax, bmin, burn_type_cutoff, xval, log_xmstar, mass_center)

            type (pgstar_hist_node), pointer :: pg
            integer, intent(in) :: i_burn_type_first, i_burn_type_last
            !real, intent(in) :: bmax, bmin, burn_type_cutoff, xval, mass, mass_center
            real, intent(in) :: bmax, bmin, burn_type_cutoff, xval, log_xmstar, mass_center

            real :: burn_qbot, burn_qtop, mbot, mtop, xmass, column_bot, column_top, area
            real :: burn_type, color_frac
            integer :: k, colormap_index, clr, mid_map
            include 'formats'
            !xmass = mass - mass_center
            xmass = real(exp10(dble(log_xmstar))/Msun)
            burn_qbot = 0
            ...
            do k = i_burn_type_first, i_burn_type_last, 2
               ...
               if (abs(burn_type) > burn_type_cutoff) then
                  ...
                  call pgsci(clr)
                  if (.not. s% Kipp_switch_to_Column_Depth) then
                     mbot = mass_center + xmass*burn_qbot
                     mtop = mass_center + xmass*burn_qtop
                     call draw1(xval, mbot, mtop, clr)
                  else
                     area = 4*pi*pow2(12.0d5)
                     column_bot = (1-burn_qbot)*xmass*Msun/area
                     column_top = (1-burn_qtop)*xmass*Msun/area
                     call draw1(xval, log10(column_bot), log10(column_top), clr)
                  end if
               end if
               burn_qbot = burn_qtop
         ...
         subroutine draw_mix_for_step( &
               pg, step, i_mix_type_first, i_mix_type_last, xval, log_xmstar, mass_center)
            type (pgstar_hist_node), pointer :: pg
            integer, intent(in) :: step, i_mix_type_first, i_mix_type_last
            ! real, intent(in) :: xval, mass, mass_center
            real, intent(in) :: xval,log_xmstar, mass_center
            real :: mix_qbot, mix_qtop, mbot, mtop, xmass, column_bot, column_top, area
            integer :: k, mix_type
            include 'formats'
            mix_qbot = 0
            ! xmass = mass - mass_center
            ...
            do k = i_mix_type_first, i_mix_type_last, 2
               ...
               if (mix_type > 0 .and. mix_type <= max_mix_type_to_show) then
                  if (.not. s% Kipp_switch_to_Column_Depth) then
                     ...
                     area = 4*pi*pow2(12.0d5)
                     column_bot = (1-mix_qbot)*xmass*Msun/area
                     column_top = (1-mix_qtop)*xmass*Msun/area
                     call draw1(xval, log10(column_bot), log10(column_top), mix_clr(mix_type))
                  endif
